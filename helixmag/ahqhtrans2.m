...
% quad coils and it is a rectangular shape.  Thus it is offset in both the z and x axis
clear all
%close all
...
zdd=7.3+0.8+.25+0;%5+2.6+0.75+0.25+.2;
...
%%%%corner coordinates%%%%
x2=1.5/100;y1=-1.2/100;y2=1.200001/100;zl=3/100;
z2=2*zdd/1000;
...
testpointx=0.0/100;%0.5/100 -> tight 0.23/100 -> high bias experiment
testpointy=-0.0/100;
addedbias=0e-4;
...
for ss=1:numz2s
   ...
   y2temp=y2+(thick2z/2)-(thick2z*(ss-1)); 
      y1temp=y1-(thick2z/2)+(thick2z*(ss-1)); 

        %%%coil on positive side %%%
     % length of wire along former face%
B(counter,(4:6))=B(counter,(4:6))+(I2*linemag(k,x2,x2,y1temp,y2temp,z2temp,z2temp,Axx,Ayy,Azz,intsteps)); 
    % length of wire perpendicular to former face%
B(counter,(4:6))=B(counter,(4:6))+(I2*linemag(k,x2,x2,y2temp,y2temp,z2temp,z2temp+zl+thick2z/2,Axx,Ayy,Azz,intsteps)); 
     % length of wire along former face, but near back of coil%
%B(counter,(4:6))=B(counter,(4:6))+(I2*linemag(k,x2,x2,y2temp,y1temp,z2temp+zl+thick2z/2,z2temp+zl+thick2z/2,Axx,Ayy,Azz,intsteps)); 
  % length of wire perpendicular to former face%
B(counter,(4:6))=B(counter,(4:6))+(I2*linemag(k,x2,x2,y1temp,y1temp,z2temp+zl+thick2z/2,z2temp+thick2z/2,Axx,Ayy,Azz,intsteps)); 

          %%%coil on negative side %%%
  % length of wire along former face%
B(counter,(4:6))=B(counter,(4:6))+(-I2*linemag(k,x2,x2,y2temp,y1temp,-z2temp,-z2temp,Axx,Ayy,Azz,intsteps)); 
    % length of wire perpendicular to former face%
B(counter,(4:6))=B(counter,(4:6))+(-I2*linemag(k,x2,x2,y1temp,y1temp,-z2temp,-(z2temp+zl+thick2z/2),Axx,Ayy,Azz,intsteps)); 
     % length of wire along former face, but near back of coil%
%B(counter,(4:6))=B(counter,(4:6))+(I2*linemag(k,x2,x2,y1temp,y2temp,-(z2temp+zl+thick2z/2),-(z2temp+zl+thick2z/2),Axx,Ayy,Azz,intsteps)); 
  % length of wire perpendicular to former face%
B(counter,(4:6))=B(counter,(4:6))+(-I2*linemag(k,x2,x2,y2temp,y2temp,-(z2temp+zl+thick2z/2),-(z2temp+thick2z/2),Axx,Ayy,Azz,intsteps)); 
