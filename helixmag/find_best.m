...
R1=.675/100;%4.6228
I1=20*(1-0); %20 -> tight, 15 -> high bias for experiment
N1=14;%23.5
...
z2=2*zdd/1000;
R2=0.7/100;%4.6228
I2=-20.8*(1+0); %20 -> tight, 30 -> high bias for experiment
N2=19 ;%23.5
...
testpointx=(0.5)/100+cent;%0.5/100 -> tight 0.23/100 -> high bias experiment
...
cent=B(t,3);
z=B(t-30:t+30,3);
plot(z-cent,Bmag(t-30:t+30))
BB=min(Bmag(t-30:t+30));
b=Bmag(t-30:t+30)-BB;
zz=z-cent;
% now lets fit it   
[Hemass,Hegamma,Helambda,Helife,HeIs,Hemu,hbar,kb,Hek,g]=Heconst;
   ...
   w=f*2*pi;
fp(1)=w;
fitt=fminsearch('harm_fit',fp,[],zz,b); 
    v=(0.5*Hemass*fitt(1)^2*zz.^2)/(Hemu*2*pi*hbar);
     plot(zz,b,'ok',zz,v,'r')
     fitt(1)/(2*pi)
    BB