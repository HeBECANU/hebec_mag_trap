...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%WE  BELIEVE THE FOLLOWING IS FOR THE CURRENT COILS AT 
%%%%%%%%%%%%%%%%%ANU AS AT 2014%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%YES THE ONES THAT EVEN SEAN USED!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
zdd=4.6+2.7+0.8+.4+.25+dz_alpha;%5+2.6+0.75+0.25+.2;
z1=2*zdd/1000; % distance just before the first wire i.e. ->|o
R1=.75/100;%4.6228
I1=0;%14.2*(1-0); %20 -> tight, 15 -> high bias for experiment
N1=10;%23.5
...
z2=2*zdd/1000;
R2=0.7/100;%4.6228
I2=-18%-13.8*(1+0); %20 -> tight, 30 -> high bias for experiment
N2=18 ;%23.5
...
testpointx=.60/100;%0.5/100 -> tight 0.23/100 -> high bias experiment
testpointy=.1/100;
testpointz=0/100;
addedbias=.0e-4;
...
addgrad=0;
if addgrad==1
    bdash=35; % gradient in Guass/m
    Bg=(B(:,3)+range/1000)*bdash/1e4;
    B(:,6)=B(:,6)+Bg;
end
