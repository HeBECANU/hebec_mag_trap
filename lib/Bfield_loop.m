...
ell_k2=4*R*rho./((R+rho).^2+z.^2);      % k^2 elliptic parameter
[K,E]=ellipke(ell_k2);      % complete elliptic integrals of 1st,2nd orders

Bzz=(mu_0*I./(2*pi)).*(1./sqrt((R+rho).^2+z.^2)).*(K+E.*(R^2-rho.^2-z.^2)./((R-rho).^2+z.^2));
Brr=(mu_0*I./(2*pi*rho)).*(z./sqrt((R+rho).^2+z.^2)).*(-K+E.*(R^2+rho.^2+z.^2)./((R-rho).^2+z.^2));
